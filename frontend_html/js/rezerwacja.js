...
async function zaladujUslugi() {
  try {
    ...
    if (uslugi.length === 0) {
      lista.innerHTML = "<p style='color:#888;font-size:14px;'>Brak dostępnych usług.</p>";
      return;
    }
...
function przeliczCene() {
  ...
  document.getElementById("cenaBiletu").innerText = cenaBiletu.toFixed(2) + " PLN";
  document.getElementById("cenaBagazu").innerText = cenaBagazu.toFixed(2) + " PLN";
  document.getElementById("cenaUslug").innerText = cenaUslug.toFixed(2) + " PLN";
...
async function utworzRezerwacje() {
  ...
  if (!idPasazera) {
    komunikat.style.color = "#ffcccc";
    komunikat.innerText = "Nie znaleziono Twojego konta pasażera. Zaloguj się ponownie.";
    return;
  }
...
document.getElementById("rezerwujBtn").addEventListener("click", utworzRezerwacje);
